...
train_df = pd.read_csv("/kaggle/input/sentiment-analysis-2/Reviews.csv")
train_df = train_df.loc[0:9999,["Score","Text"]]
test_df = pd.read_csv("/kaggle/input/sentiment-analysis-2/test_2.csv")
print(test_df)
test_x = test_df.loc[:,"Text"]
def score_map(x):
    score = 1 if x >=4 else 0
    return score
